...
app.use((err: MiddlewareError, req: Request, res: Response, next: NextFunction) => {
  ...
  const defaultClientError: MiddlewareError = new MiddlewareError('Unknown server error. Please check server log.', 500);
  const clientError: MiddlewareError = (err instanceof MiddlewareError)
